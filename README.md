...
- `app/db/`: SQLAlchemy base and session wiring.
- `app/models/`: ORM models for candidates, jobs, applications, scores.
...
3. Ensure `DATABASE_URL` in `.env` points to the external PostgreSQL database.
4. Apply migrations:
...
Login uses fixed week-1 credentials against the external database-backed API:
